...
# - 07. Oct. 2011  - Adapted for PA Project Rotating LED-Display ZHAW
...
SRC += src/startup_stm32f10x_md_mthomas.c
SRC += src/syscalls.c

SRC += src/errdisplay.c
...
BUILDONCHANGE = Makefile src/platform_config.h
...
program: $(OUTDIR)/$(TARGET).bin
	...
	tools/stm32loader.py -evw -p /dev/ttyUSB0 $(OUTDIR)/$(TARGET).bin
