...
static int usbGetStringAscii(usb_dev_handle *dev, int index, int langid, char *buf, int buflen) {
	...
	if ((rval = usb_control_msg(dev, USB_ENDPOINT_IN, USB_REQ_GET_DESCRIPTOR, (USB_DT_STRING << 8) + index, langid,
...
int usbSetReport(usbDevice_t *device, int reportType, char *buffer, int len) {
	...
	bytesSent = usb_control_msg(device, USB_TYPE_CLASS | USB_RECIP_INTERFACE | USB_ENDPOINT_OUT, USBRQ_HID_SET_REPORT,
...
int usbWriteString(usbDevice_t *device, char *buffer, int len) {
	int bytesSent;
	int reportType = 0;
	...
	bytesSent = usb_bulk_write(device, 2, buffer, len, 5000);
	if (bytesSent != len) {
		if (bytesSent < 0)
			fprintf(stderr, "Error sending message: %s\n", usb_strerror());
		return USB_ERROR_IO;
...
int usbGetReport(usbDevice_t *device, int reportType, int reportNumber, char *buffer, int *len) {
	...
	bytesReceived = usb_control_msg(device, USB_TYPE_CLASS | USB_RECIP_INTERFACE | USB_ENDPOINT_IN,
			USBRQ_HID_GET_REPORT, reportType << 8 | reportNumber, 0, buffer, maxLen, 5000);
	if (bytesReceived < 0) {
		fprintf(stderr, "Error sending message: %s\n", usb_strerror());
		return USB_ERROR_IO;
	}
